...
function rs4tot(data,fileinfo,description,header2,groupvars,rgroupvars,ancillifo,EdChanFind)
        ...
        while plotrec
            for ipar=1:4
                ...
                for i=1:nchans
                    ...
                    if ierd==0
                        if ipar==4
                            vecp=(squeeze(data(i,1,2:ilnd,:)).^2)./squeeze(data(i,2,2:ilnd,:));
                            mxpv=max(max(max((squeeze(data(:,1,2:ilnd,:)).^2)./squeeze(data(:,2,2:ilnd,:)))));
                        else
                            vecp=squeeze(data(i,ipar,2:ilnd,:));
                            mxpv=max(max(max(data(:,ipar,2:ilnd,:))));
                        end
                        plot(squeeze(data(i,4,2:ilnd,:)),vecp,'b.','MarkerSize',20);
                        %h=line([squeeze(data(i,4,2:ilnd,:))'; squeeze(data(i,4,2:ilnd,:))'],[zeros(1,ilnd-1); vecp'],...
                ...
                set(gca,'Visible','off');
                MainTitle=sprintf('FAD PARAMETER %s, ALL RECORDS',PARNAME{ipar});
