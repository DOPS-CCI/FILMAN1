! SETUP.FOR- SUBROUTINE TO ALLOCATE PLOT REGIONS FOR MULTICHANNEL
! PLOTS. NASTY ROUTINE BUT ONLY EXECUTES ONCE. MODIFIED FROM PLOT.FOR
! VERSION. STILL NEED TO CHECK #CHANS(VARIES)
...
SUBROUTINE SETUP(IXDIM,IYDIM,MARGIN,CLABEL,NC,MODE, &  ! PASSED
	...
	! DEFINE TOTAL AVAILABLE SCREEN DIMENSIONS
...
4   NMISC=0           ! NON-SCALP CHANNELS
	DO 10 I=1,NC
	    ...
	    MISC(NMISC)=I
10	    CONTINUE
! DETERMINE NUMBERS OF ROWS & COLUMNS TO PLOT
! DO ROWS FIRST; THEY'LL INCLUDE ANY NON-SCALP CHANNELS
	MAP=0
	ASSIGN 201 TO NEXT
...
! NOW ASSIGN FINAL PLOT ROW POSITIONS TO THE SELECTED SCALP CHANNELS
	DO 25 I=1,NC
	    K=ISTART(I,1)
	    IF(K.LE.0) GO TO 25
	    ISTART(I,1)=MAP(K)
25	    CONTINUE
! NOW PROCESS THE COLUMN COORDINATES IN THE SAME WAY
	MAP=0
	NCOL=0
	DO 30 I=1,NC
	K=ISTART(I,2)
...
30	CONTINUE
	GO TO 16
! NOW ASSIGN COLUMN POSITIONS TO THE SCALP CHANNELS
31	DO 35 I=1,NC
	    K=ISTART(I,2)
	    IF(K.LE.0) GO TO 35
	    ISTART(I,2)=MAP(K)
35	    CONTINUE
! DETERMINE WHERE TO PUT THE MISCELLANEOUS ONES:
! WILL THEY FIT IN LAST SCALP ROW?
	I2MAX=0
	DO 40 I=1,NC
...
441	    ISTART(MISC(I),1)=NR
	    ISTART(MISC(I),2)=NM1
45	    NM1=NM1+1
! FINALLY, TRANSLATE PLOT ROW,COLUMN POSITIONS TO SCREEN  COORDINATES,
! DETERMINE XDIM & YDIM , AND RETURN
	IXT=XTOT/FLOAT(NCOL)
...
50	    ISTART(I,1)=MARGIN+IC*IXT      ! HORIZONTAL STARTPOINT
	RETURN
END

! SUBROUTINE TO SELECT LABELING SCHEME FOR ABSCISSA
...
5	STEP=.125
	XINT=1.0
! CANONICAL VALUES ARE FOR <16HZ CASE; ADJUST FOR OTHERS
	IRANGE=FLOAT(N-1)*DX
	IR=8
	FACT=.5
	DO 10 K=1,8
	    FACT=2*FACT
	    IR=2*IR
	    IF(IR.GE.IRANGE) GO TO 15
10	CONTINUE
15	STEP=STEP*FACT
	XINT=XINT*FACT
20	IF(XDIM.GE.(IXTOT-MARGIN)) GO TO 30
! CORRECT FOR PLOT'S ACTUAL LENGTH IN MULTICHANNEL CASES
	ISIZE=IXTOT/(INT(XDIM)+MARGIN)      ! INTEGER MULTIPLIER
! CHANGE IT TO NEXT HIGHER POWER OF 2      
	M=-1
21	M=M+1
...
! ***************
! SUBROUTINE TO EXTRACT POSITION INFO FROM FILMAN LABEL FIELDS; MAY
! NEED TIGHTENING UP TO CHECK FOR IMPOSSIBLE VALUES ETC
...
! IF NOT A SCALP CHANNEL SETS IROW, ICOL TO 0
	CHARACTER*24 CLABEL
	CHARACTER*1 ITBL(11)
