...
C      retlog=DlgSet(dlg,IDC_CHECK7,.FALSE.,DLG_ENABLE)
...
C      if(iidx.ne.0)then
C        TEMPFIL=INFIL(1:iidx-1)//'.FADout'//INFIL(iidx:len_trim(INFIL))
C      else
C        TEMPFIL=INFIL(1:LEN_TRIM(INFIL))//'.FADout.dat'
C      endif
C      retlog=DlgSetChar(dlg,IDC_EDIT6,TEMPFIL(1:LEN_TRIM(TEMPFIL)))
      TEMPFIL=''
      retlog=DlgSetChar(dlg,IDC_STATIC15,' ')
      
!      retlog=DlgSet(dlg,IDC_RADIO4,.FALSE.,DLG_ENABLE) ! Not yet ready
!      retlog=DlgSet(dlg,IDC_RADIO5,.FALSE.,DLG_ENABLE) ! to allow for
!      retlog=DlgSet(dlg,IDC_RADIO8,.FALSE.,DLG_ENABLE) ! different algorithms


C      retlog=DlgSetLog(dlg,IDC_CHECK10,.FALSE.)  ! Normalize over time
      retlog=DlgSetLog(dlg,IDC_CHECK10,.TRUE.)  ! Normalize over time
      
      NormR=.FALSE.

C      retlog=DlgSetSub(dlg,IDC_BUTTON23,LastFADFileCheck)
...
C	  retlog=DlgGet(dlg,IDC_CHECK7,ACTVALS.CHECK7,DLG_ENABLE)
...
C	  retlog=DlgSet(dlg,IDC_EDIT7,.FALSE.,DLG_ENABLE)
...
!        Ltemp=PrcSel(LineLong,NR,IVAL,3)
!        do 21,IGRU=1,IVAL
!21      selgrps(IGRU)=numbrs(IGRU)
      elseif(ians.eq.8)THEN
        retlog=DlgGetChar(dlg,IDC_EDIT2,LINELONG)
!        Ltemp=PrcSel(LineLong,NR,IVAL,3)
!        do 22,IGRU=1,IVAL
!22      selgrps(IGRU)=numbrs(IGRU)
      ENDIF
...
!      SUBROUTINE CheckValueNumber(dlg,id,callbacktype)
!      use iflogm
!      include 'resource.fd'
!      type (dialog) dlg
!      integer id
...
!        retlog=DlgSetChar(dlg,GRPEDS(indx),LINE)
!        len1=LEN_TRIM(LINE)
!        retlog=DlgSetChar(dlg,VALEDS(indx),LINE)
!        len2=LEN_TRIM(LINE)
!        ISUM=ISUM+len1*len2
!  2   CONTINUE
!      IF(ISUM.EQ.0)THEN
...
C      retlog=DlgSet(dlg,IDC_STATIC5,64,DLG_TEXTLENGTH)
...
C      retlog=DlgSet(dlg,IDOK,.FALSE.,DLG_ENABLE)
