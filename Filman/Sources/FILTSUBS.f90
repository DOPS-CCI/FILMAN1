...
50	        WORK(LS,1)=WORK(LS,1)+TMPC*(W02**K)*TMP
	DO 55 LL=0,L
	    D(LL)=WORK(LL,0)
55	    C(LL)=WORK(LL,1)
!-SUBSTITUTE (Z-1)/(Z+1)
100	CALL SPBILN(D,C,LN,B,A,WORK,IERROR)
	RETURN
END

SUBROUTINE SPBILN(D,C,LN,B,A,WORK,IERROR)
!-LATEST DATE: 11/13/85
...
6       WORK(I,0)=TMP
	DO 7 I=1,L
	    DO 7 J=1,L
7	        WORK(I,J)=WORK(I,J-1)-WORK(I-1,J)-WORK(I-1,J-1)
	DO 10 I=L,0,-1
	    B(I)=0.
	    ATMP=0.
	    DO 9 J=0,L
	        B(I)=B(I)+WORK(I,J)*D(J)
9	        ATMP=ATMP+WORK(I,J)*C(J)
	    SCALE=ATMP
	    IF(I.NE.0) A(I)=ATMP
10	    CONTINUE
	IERROR=2
...
!
! FILTERS N-POINT ARRAY IN PLACE USING TYPE II TRANSPOSED
! 
! B = ARRAY OF NUM COEFFICIENTS
! A = ARRAY OF DENOM COEFFICIENTS
! LB = LENGTH OF B, LA = LENGTH OF A
! X = DATA TO BE FILTERED
! N = LENGTH OF X
! Z = INITIAL CONDITIONS OF DELAYS
!
...
!-    1 - NS  OUTPUT AT STAGE [IERROR] EXCEEDS 1.E10
	...
	DO 3 M=1,NS
	    ...
	    DO 3 K=0,N-1
	        ...
	        X(K)=B(0,M)*PX(0,M)
	        DO 1 LL=1,LS
1	            X(K)=X(K)+B(LL,M)*PX(LL,M)-A(LL,M)*PY(LL,M)
	        IF(ABS(X(K)).GT.1.E10) RETURN
	        DO 2 LL=LS,2,-1
	            PX(LL,M)=PX(LL-1,M)
2	            PY(LL,M)=PY(LL-1,M)
