...
PUBLIC XGetWindowPos
PUBLIC XSetWindowPos
PUBLIC XGetWindowRect       !(xWnd, iX1, iY1, [iWidth], [iHeight], [bParent])

INTEGER,PARAMETER::        XPROC_NONE        = -2   !Unbound XFT window

TYPE,PUBLIC:: X_CONTROL
      SEQUENCE
      INTEGER:: hWnd = 0
      INTEGER:: iType = 0
      INTEGER:: iStyle = 0
...
!      TYPE(X_CONTROL),POINTER::   xCtl(:) => NULL()
      INTEGER,POINTER::   daMsg(:)      
      INTEGER,POINTER::   daHandler(:)  
      INTEGER,POINTER::   daCommand(:)  
      INTEGER,POINTER::   daCmdHandler(:)
      TYPE(X_CONTROL),POINTER::   xCtl(:) 
!<NEW>
      INTEGER::            iCanvasX = 0
      INTEGER::            iCanvasY = 0
!</NEW>
