...
INTEGER, PARAMETER:: SHDID_NET_OTHER                 = 17
!</delete>
...
PUBLIC  XSystemPath           !(xWnd, iCSIDL, sDir)
PUBLIC  XEnvironmentString    !(sVariable, sString)
...
IF (PRESENT(sExts)) THEN
      ...
      IF (szExt .eq. '*') OFN%lpstrDefExt=NULL
ELSE
      OFN%lpstrDefExt=NULL
END IF
OFN%lCustData=0
...
CHARACTER(64), AUTOMATIC::             szTitle
CHARACTER(LEN=MAX_PATH)::              sDirTmp
...
IF (IAND(jFlags,BIF_BROWSEFORCOMPUTER).NE.0) THEN
      sDirTmp = m_szDir
ELSE
      XBrowse = SHGetPathFromIDList(lpIDList, sDirTmp)
END IF
...
hFindFile = FindFirstFile(TRIM(sDir)//"\"//TRIM(sFilter)//CHAR(0), WFDfile)
...
INTEGER FUNCTION XCreateShortcut(sTarget, sShortcut,                   &
                  sDescription, sArguments, sIconPath, iIconIndex,     &
                  sWorkingDir) RESULT (hr)
...
CHARACTER(*), OPTIONAL, INTENT(IN)::   sWorkingDir    !Optional working directory
...
INTERFACE
      INTEGER FUNCTION IShellLink_SetWorkingDirectory(psl, szDir)
      !DEC$ATTRIBUTES STDCALL:: IShellLink_SetWorkingDirectory
      !DEC$ATTRIBUTES REFERENCE:: szDir
      INTEGER(4)::   psl
      CHARACTER(*):: szDir
      END FUNCTION
END INTERFACE
POINTER(lpIShellLink_SetWorkingDirectory,IShellLink_SetWorkingDirectory)
...
IF (hr.EQ.0) THEN
      ...
      END IF
      IF (PRESENT(sWorkingDir)) THEN
            lpIShellLink_SetWorkingDirectory = DEREFERENCE_PROC(psl, 9)
            hr = IShellLink_SetWorkingDirectory(psl, TRIM(sWorkingDir)//CHAR(0))
      END IF
...
!======================================================================
!XSystemPath returns the directory associated with a given CSIDL value
LOGICAL FUNCTION XSystemPath(xWnd, iCSIDL, sDir)
...
TYPE (X_WINDOW), INTENT(IN)::          xWnd
!DEC$ENDIF
INTEGER, INTENT(IN)::                  iCSIDL
CHARACTER(*), INTENT(OUT)::            sDir

INTEGER::                              iSt, lpIDList
CHARACTER(LEN=MAX_PATH)::              sDirTmp
...
!DEC$ENDIF
      ...
      iSt = SHGetSpecialFolderLocation(m_hParent, iCSIDL, lpIDList)

!        SHGetPathFromIDList fails (returns false) if given a virtual directory
      XSystemPath = SHGetPathFromIDList(lpIDList, sDirTmp)

CALL CoTaskMemFree(lpIDList)

sDir = ''
IF (XSystemPath) sDir=XC2F(sDirTmp)

END FUNCTION XSystemPath

!======================================================================
!XEnvironmentString returns the string associated with a given environment variable
! Or alternately, the module filename
      LOGICAL FUNCTION XEnvironmentString (sVariable, sString)
      CHARACTER(*), INTENT(IN)  :: sVariable
      CHARACTER(*), INTENT(OUT) :: sString
!
!          local store
      INTEGER LPD

      XEnvironmentString = .FALSE.

      IF (sVariable .EQ. ' ') THEN
!           if blank sVariable, return module filename
         LPD = GetModuleFileName (0, sString, LEN (sString))

      ELSE
!           see if %name% form given
         IF (INDEX (sVariable, '%') .GT. 0) THEN
            LPD = ExpandEnvironmentStrings (TRIM(sVariable) // CHAR (0), &
     &              sString, LEN (sString)) - 1   ! the -1 is to exclude null
         ELSE
            LPD = GetEnvironmentVariable (TRIM(sVariable) // CHAR (0), &
     &              sString, LEN (sString))
         ENDIF
...
!           check for error
      IF (LPD .LE. 0) THEN
         sString = ' '           ! not found
         RETURN
      ELSE IF (LPD .GT. LEN (sString)) THEN
         sString = 'TOO SMALL'            ! not enuff room for answer
         RETURN
      ELSE IF (LPD .LT. LEN (sString)) THEN
         sString(LPD+1:) = ' '            ! remove trailing null
      ENDIF
!           check for failure to substitute
      IF (INDEX (sVariable, '%') .GT. 0 .AND. sVariable .EQ. sString) THEN
         sString = ' '           ! substitute not found
         RETURN
      ENDIF

      XEnvironmentString = .TRUE.

      END FUNCTION XEnvironmentString
...
!======================================================================
!DEREFERENCE_PROC returns the address of the (zero-based) iOrder-th routine
!in the interface by looking at the interface's vtable
...
lpInterface = lpObject     !address of interface is 1st dword in object's address
lpVTable = iInterface      !address of vtable is 1st dword in object's interface
lpVTableEntry = lpVTable + 4*iOrder !iOrder-th dword points to the routine address
lpProc = VTableEntry
